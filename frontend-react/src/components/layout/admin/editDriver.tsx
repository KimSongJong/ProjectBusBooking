...
import type { UpdateDriverRequest } from "@/types/driver.types"
...
interface EditDriverDialogProps {
  ...
  formData: UpdateDriverRequest
  onInputChange: (field: keyof UpdateDriverRequest, value: string | number) => void
...
export default function EditDriverDialog({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload ảnh mới nếu có file chưa upload
      if (imageFile) {
        setUploading(true)
  ...
  return (
    <Dialog open={open} onOpenChange={handleDialogClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-slate-800">Chỉnh sửa tài xế</DialogTitle>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {uploading ? "Đang upload..." : imageFile ? "Upload ảnh mới" : "Chọn ảnh để thay đổi"}
                </Button>
              ...
              {imageFile && (
                <div className="text-xs text-blue-600 bg-blue-50 p-2 rounded">
                  ℹ Nhấn "Upload ảnh mới" để thay thế ảnh hiện tại
                </div>
              )}
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            >
              <FaSave /> {uploading ? "Đang xử lý..." : "Cập nhật"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
